...
def translation_generator(perimeter, location):
    ...
    lat = h/lat_conversion * -1 ^ randint(0,1)
    lng = w/lng_conversion * -1 ^ randint(0,1)
    return [lat, lng]

"""takes an origin point and a distance length and returns a dictionary of directions of points to tuples of coordinates"""
...
def points_formatter(coordinates):
    formattedPoints = ''
...
def generate_path(points):
    ...
    resp = requests.get(url, params = {'path': points})
    return list(resp)
